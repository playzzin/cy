...
const ConstructionCompanyDatabase: React.FC<ConstructionCompanyDatabaseProps> = ({
    ...
    const handleUpdateCompany = async () => {
        if (!editingCompany?.id || !formData.name) {
            alert('회사명은 필수입니다.');
            return;
        }
            ...
            <div className="mb-8">
                <div className={`flex items-center mb-6 ${hideHeader ? 'justify-end' : 'justify-between'}`}>
                    ...
                    <div className="flex gap-2 items-center">
                        <label className="flex items-center gap-2 cursor-pointer bg-white px-2 py-1.5 rounded border border-gray-200 hover:bg-gray-50 h-full">
                            <input
                                type="checkbox"
                    ...
                    <div className={`min-h-[500px] ${isStickyHeader ? 'overflow-auto h-[calc(100vh-300px)]' : 'overflow-x-auto'}`}>
                        <table className="w-full">
                            <thead className={`bg-slate-900 text-white ${isStickyHeader ? 'sticky top-0 z-10' : ''}`}>
                                <tr>
                                    <th className="px-4 py-3 w-4 border border-slate-700">
                                        <input
                                            type="checkbox"
                                            checked={filteredCompanies.length > 0 && selectedCompanyIds.length === filteredCompanies.length}
                                            onChange={toggleSelectAll}
                                            className="rounded text-indigo-600 focus:ring-indigo-500 w-4 h-4 cursor-pointer bg-slate-800 border-slate-600"
                                        />
                                    </th>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-white uppercase tracking-wider border border-slate-700">
                                        발주사/건설사
                                    </th>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-white uppercase tracking-wider border border-slate-700">
                                        대표자
                                        ...
                                        <React.Fragment key={company.id}>
                                            <tr
                                                ...
                                                ref={isHighlighted
                                                    ...
                                                    : null}
                                            >
                                                <td className="px-4 py-4 border border-slate-200" onClick={(e) => e.stopPropagation()}>
                                                    <input
                                                        type="checkbox"
                                                ...
                                                <td className="px-4 py-4 border border-slate-200">
                                                    <InputPopover
                                                        ...
                                                        onChange={(val) => handleCompanyFieldUpdate(company.id!, 'address', val)}
                                                        placeholder="주소 입력"
                                                        minimal={true}
                                                    />
                                                </td>
                                                ...
                                                <td className="px-4 py-4 border border-slate-200">
                                                    <InputPopover
                                                        ...
                                                        onChange={(val) => handleCompanyFieldUpdate(company.id!, 'businessNumber', val)}
