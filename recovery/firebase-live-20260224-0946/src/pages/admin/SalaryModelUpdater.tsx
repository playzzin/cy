    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            workers.forEach((worker) => {
                ...
                const before = String(worker.salaryModel ?? worker.payType ?? '');
                const correctModel = SALARY_MODEL_MAP[name];
                const id = worker.id ? String(worker.id) : '';
                if (!correctModel || !id) return;
                if (before === correctModel) return;

                updateResults.push({ name, before: before || '(없음)', after: correctModel });
                const arr = idsByModel.get(correctModel) ?? [];
                arr.push(id);
                idsByModel.set(correctModel, arr);
            });

            for (const [model, ids] of idsByModel.entries()) {
                if (ids.length === 0) continue;
                await manpowerService.updateWorkersBatch(ids, { salaryModel: model });
            }
