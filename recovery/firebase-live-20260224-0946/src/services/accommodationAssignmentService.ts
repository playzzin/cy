...
const ASSIGNMENT_COLLECTION_CANDIDATES = ['accommodation_assignments', 'accommodationAssignments'];
...
const loadFirestoreAssignments = async (): Promise<any[]> => {
    for (const collectionName of ASSIGNMENT_COLLECTION_CANDIDATES) {
        try {
...
const loadDcAssignments = async (): Promise<void> => {
    ...
    let rows = (res as any)?.data?.accommodationAssignments ?? [];

    const isSparseDcRow = (row: any): boolean => {
        const hasAccommodationRef = Boolean(row?.accommodation?.id || row?.accommodationId);
        const hasWorkerRef = Boolean(row?.worker?.id || row?.workerId);
        const hasTeamRef = Boolean(row?.team?.id || row?.teamId || row?.teamName);
        const hasSource = row?.source === 'team' || row?.source === 'worker';
        return !(hasAccommodationRef && hasWorkerRef && hasTeamRef && hasSource);
    };

    const shouldHydrateFromFirestore = rows.length === 0 || rows.every((row: any) => isSparseDcRow(row));
    if (shouldHydrateFromFirestore) {
        const firestoreRows = await loadFirestoreAssignments();

        if (rows.length === 0 && firestoreRows.length > 0) {
            rows = firestoreRows.map((fs) => ({
                id: fs?.id,
                legacyId: fs?.legacyId ?? null,
                worker: fs?.workerId ? { id: String(fs.workerId) } : undefined,
                workerName: fs?.workerName ?? null,
                team: fs?.teamId ? { id: String(fs.teamId) } : undefined,
                teamName: fs?.teamName ?? null,
                accommodation: fs?.accommodationId ? { id: String(fs.accommodationId) } : undefined,
                accommodationName: fs?.accommodationName ?? null,
        ...
        } else if (rows.length > 0 && firestoreRows.length > 0) {
            const fsById = new Map<string, any>();
            const fsByLegacyId = new Map<string, any>();
            const fsByComposite = new Map<string, any>();

            firestoreRows.forEach((fs) => {
                const id = fs?.id ? String(fs.id) : '';
                const legacyId = fs?.legacyId ? String(fs.legacyId) : '';
                const key = `${String(fs?.workerName ?? '').trim()}|${String(fs?.accommodationName ?? '').trim()}|${String(fs?.startDate ?? '').trim()}`;
                if (id) fsById.set(id, fs);
                if (legacyId) fsByLegacyId.set(legacyId, fs);
                if (key.trim() && !fsByComposite.has(key)) fsByComposite.set(key, fs);
            });

            rows = rows.map((row: any) => {
                const id = row?.id ? String(row.id) : '';
                const legacyId = row?.legacyId ? String(row.legacyId) : '';
                const compositeKey = `${String(row?.workerName ?? '').trim()}|${String(row?.accommodationName ?? '').trim()}|${String(row?.startDate ?? '').trim()}`;
                const fs = (id ? fsById.get(id) : undefined)
                    ?? (legacyId ? fsByLegacyId.get(legacyId) : undefined)
                    ?? (compositeKey.trim() ? fsByComposite.get(compositeKey) : undefined);

                if (!fs) return row;

                return {
                    ...row,
                    legacyId: row?.legacyId ?? fs?.legacyId ?? null,
                    worker: row?.worker ?? (fs?.workerId ? { id: String(fs.workerId) } : undefined),
                    workerId: row?.workerId ?? (fs?.workerId ? String(fs.workerId) : undefined),
                    workerName: row?.workerName ?? fs?.workerName ?? null,
                    team: row?.team ?? (fs?.teamId ? { id: String(fs.teamId) } : undefined),
                    teamId: row?.teamId ?? (fs?.teamId ? String(fs.teamId) : undefined),
                    teamName: row?.teamName ?? fs?.teamName ?? null,
                    accommodation: row?.accommodation ?? (fs?.accommodationId ? { id: String(fs.accommodationId) } : undefined),
                    accommodationId: row?.accommodationId ?? (fs?.accommodationId ? String(fs.accommodationId) : undefined),
                    accommodationName: row?.accommodationName ?? fs?.accommodationName ?? null,
                    source: row?.source ?? fs?.source ?? null,
                    memo: row?.memo ?? fs?.memo ?? null,
                    updatedAt: row?.updatedAt ?? fs?.updatedAt ?? null
                };
            });
        }
    }
...
export const accommodationAssignmentService = {
    ...
    addAssignmentsBatch: async (
        ...
        await Promise.all([loadDcWorkers(), loadDcTeams(), loadDcAccommodations()]);
