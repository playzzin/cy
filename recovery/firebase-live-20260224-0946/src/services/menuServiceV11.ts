...
export const menuServiceV11 = {
    ...
    runOneTimeMigrations: async () => {
        ...
        if (flags.dataManagementConsoleMenuV1) return false;

        const config = await menuServiceV11.getMenuConfig({ allowFallback: false });
        if (!config) return false;

        const ensured = ensureDataManagementConsoleMenuExists(config);
        if (ensured.modified) {
            await menuServiceV11.saveMenuConfig(ensured.config);
        }

        await saveMigrationFlags({ ...flags, dataManagementConsoleMenuV1: true });
