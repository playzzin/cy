...
export interface PayrollData {
    ...
    name: string;
    role: string;
    address?: string; // Added for Labor Cost Invoice
    gongsu: {
        total: number;
        reported: number;
        labor: number;
...
export const payrollService = {
    async getPayrollData(year: number, month: number, teamId?: string, siteId?: string): Promise<PayrollData[]> {
        try {
            ...
            // Fetch Workser Master Data to check Salary Model and Address
            const { manpowerService } = await import('./manpowerService');
            const workersMaster = await manpowerService.getWorkers();
            const workerMasterMap = new Map<string, any>();
            workersMaster.forEach((w: any) => {
                if (w?.id) workerMasterMap.set(String(w.id), w);
                if (w?.legacyId) workerMasterMap.set(String(w.legacyId), w);
            });
                    ...
                    report.workers.forEach((worker: DailyReportWorker) => {
                        ...
                        if (!workerMap.has(workerId)) {
                            workerMap.set(workerId, {
                                ...
                                address: masterData?.address || '', // Fetch Address
                                totalManDay: 0,
                                unitPrice: unitPrice,
                                totalGross: 0,
            ...
            const payrollList: PayrollData[] = Array.from(workerMap.values()).map(worker => {
                ...
                return {
                    ...
                    name: worker.name,
                    role: worker.role,
                    address: worker.address, // Populate address
                    gongsu: {
