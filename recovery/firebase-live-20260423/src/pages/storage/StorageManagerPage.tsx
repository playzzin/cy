...
const StorageManagerPage: React.FC = () => {
    ...
    // Moved processRename to end of function body to fix scope issue


    const processRename = async (item: StorageItem) => {
        const { value: newName } = await Swal.fire({
            title: '이름 변경',
            input: 'text',
            inputValue: item.name,
            inputLabel: '새 이름을 입력하세요',
            showCancelButton: true,
            confirmButtonText: '변경',
            cancelButtonText: '취소',
            inputValidator: (value) => {
                if (!value) return '이름을 입력해주세요!';
                if (value.includes('/')) return '/ 문자는 사용할 수 없습니다.';
                if (value === item.name) return '기존 이름과 동일합니다.';
                return null;
            }
        });
        ...
        if (newName) {
            ...
            setItems(curr => curr.map(i =>
                i.fullPath === item.fullPath
                    ? { ...i, name: newName, fullPath: i.fullPath.replace(item.name, newName) }
                    : i
            ));

            try {
                await storageService.rename(item.fullPath, newName, item.isFolder);
                Swal.fire({
                    icon: 'success',
                    title: '변경 완료',
                    toast: true,
                ...
                loadItems(currentPath); // Refresh to be sure
            } catch (error) {
                setItems(prev); // Rollback
                console.error(error);
