...
export function accommodationReviewBill(doc: AccommodationBillingDocument, sourceForLine: (line: AccommodationBillingDocument['lineItems'][number]) => string | undefined): LedgerReviewBill {
    const charges = doc.lineItems.filter(line => line.status !== 'cancelled' && !line.cancelledAt).map(line => ({ sourceId: sourceForLine(line), recipient: ledgerRecipientKey(doc.issuedToType, doc.teamId, doc.issuedToWorkerId), recipientLabel: doc.issuedToWorkerName || doc.teamName, category: line.targetField, amount: line.amount, lineId: line.id }));
    return { id: doc.id, label: doc.issuedToWorkerName || doc.teamName || '숙소 청구', status: doc.status, teamId: doc.teamId, workerId: doc.issuedToWorkerId, recipientType: doc.issuedToType, postedAdvancePaymentId: doc.postedAdvancePaymentId, charges, total: charges.reduce((sum, c) => sum + c.amount, 0) };
}
