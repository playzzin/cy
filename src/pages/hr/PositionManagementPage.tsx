...
import { faUserTag, faPlus, faTrash, faEdit, faSave, faTimes, faSort, faCheck, faUserShield, faUserGear, faUser } from '@fortawesome/free-solid-svg-icons';
            ...
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
                        <FontAwesomeIcon icon={faUserTag} className="text-indigo-600" />
                        직책 관리
                    </h1>
                    <p className="text-slate-500 text-sm mt-1">
                        시스템에서 사용하는 직책을 정의하고, 각 직책별 <strong>시스템 권한(관리자/매니저/일반)</strong>을 설정합니다.
                    </p>
                </div>
                <div className="flex gap-2">
                    <button
                        onClick={async () => {
                            if (window.confirm("정말로 모든 직책을 초기화하고 기본 직책(사장, 매니저 등)으로 되돌리시겠습니까?\n기존 데이터는 삭제됩니다.")) {
                                try {
                                    await positionService.resetToDefaults();
                                    await fetchPositions();
                                    alert("직책이 초기화되었습니다.");
                                } catch (e) {
                                    alert("초기화 실패");
                        ...
                        className="px-4 py-2 bg-slate-100 text-slate-600 rounded-lg hover:bg-slate-200 transition-colors flex items-center gap-2 font-bold shadow-sm"
                    >
                    ...
                    <button
                        ...
                        className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors flex items-center gap-2 font-bold shadow-sm"
                    >
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <input
                                            ...
                                            className="w-full border-slate-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 text-sm p-2.5 border"
                                        />
                                        <span className="text-xs text-slate-500 whitespace-nowrap">낮을수록 높음 (1=사장)</span>
                                    </div>
                                </div>
