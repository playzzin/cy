...
import { faArrowUp, faSave, faRotateRight } from '@fortawesome/free-solid-svg-icons';
        ...
        <div className="p-6 max-w-[1800px] mx-auto bg-slate-50 min-h-screen">
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-slate-500 mt-1">자재 출고 내역을 병렬로 확인하고 등록합니다</p>
                </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                ...
                <div className="grid grid-cols-4 gap-4 mb-6 pb-6 border-b border-slate-200">
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">출고일자 *</label>
                        <input
                            type="date"
                    ...
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">인수자</label>
                        <input
                            type="text"
                            value={recipient}
                        ...
                        <div key={category} className="bg-slate-50 rounded-xl border border-slate-200 overflow-hidden">
                            <div className="bg-red-50/50 px-4 py-3 border-b border-red-100 flex justify-between items-center">
                                <h3 className="font-bold text-slate-700 flex items-center gap-2">
                                    <span className="bg-red-500 w-2 h-6 rounded-sm"></span>
                                    {category}
                                </h3>
                                <div className="text-xs text-red-600 font-medium bg-white px-2 py-1 rounded border border-red-100">
                                    ...
                                    <tbody className="divide-y divide-slate-200 bg-white">
                                        {categoryMaterials.map(material => {
                                            ...
                                            return (
                                                <tr
                                                    key={material.id}
                                                    className={`transition-colors ${qty > 0 ? 'bg-red-50' : 'hover:bg-slate-50'}`}
                                                >
                                                    ...
                                                    <td className="p-3 text-center">
                                                        <input
                                                            ...
                                                            placeholder="0"
                                                            className={`w-full border rounded-md px-2 py-1.5 text-center font-bold transition-all focus:ring-2 focus:ring-red-200 outline-none ${qty > 0
                                                                    ? 'border-red-500 text-red-700 bg-white'
                                                                    : 'border-slate-200 bg-slate-50 text-slate-400 focus:bg-white focus:border-red-500 focus:text-slate-800'
                                                                }`}
                ...
                <div className="flex justify-end gap-2 pt-6 border-t border-slate-200">
                    ...
                    <button
                        onClick={handleSave}
                        disabled={loading}
                        className="bg-red-600 text-white px-6 py-2 rounded-lg hover:bg-red-700 transition flex items-center gap-2 shadow-md disabled:opacity-50"
                    >
