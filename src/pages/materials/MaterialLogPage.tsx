...
import { materialLogService } from '../../services/materialLogService';
import type {
  MaterialFieldChange,
  MaterialLog,
  MaterialLogAction,
...
type ActionFilter = 'all' | MaterialLogAction;
type EntityFilter = 'all' | MaterialLogEntityType;

const ACTION_META: Record<MaterialLogAction, { label: string; icon: any; badge: string; text: string }> = {
  created: {
    label: '저장',
    icon: faPlus,
...
const ENTITY_META: Record<MaterialLogEntityType, { label: string; icon: any; badge: string }> = {
  material: {
    label: '자재 마스터',
    icon: faBoxesStacked,
...
const buildSearchText = (log: MaterialLog): string => [
  log.entityId,
  log.entityLabel,
  log.materialId,
  log.materialKey,
...
const filterLogs = (
  ...
  return logs.filter((log) => {
    ...
    if (filters.transactionDate && log.transactionDate !== filters.transactionDate) return false;
...
const FieldChangeList: React.FC<{ changes: MaterialFieldChange[] }> = ({ changes }) => {
  if (changes.length === 0) {
    return (
...
const DetailPanel: React.FC<{ log: MaterialLog; onClose: () => void }> = ({ log, onClose }) => {
  const [showJson, setShowJson] = useState(false);
  const action = ACTION_META[log.action] || ACTION_META.updated;
  const entity = ENTITY_META[log.entityType] || ENTITY_META.material;
    ...
    <aside className="rounded-xl border border-slate-200 bg-white shadow-sm">
      <div className="flex items-start justify-between gap-4 border-b border-slate-200 bg-slate-50 px-5 py-4">
        <div>
          ...
          <h2 className="mt-3 text-xl font-black text-slate-900">{log.materialName}</h2>
          <p className="mt-1 text-sm text-slate-500">{log.siteName || '현장 미지정'} · {log.transactionDate || '거래일 없음'}</p>
        </div>
        <button
          type="button"
          onClick={onClose}
      ...
      <div className="space-y-5 p-5">
        ...
        <section>
          <h3 className="mb-3 flex items-center gap-2 font-black text-slate-900">
            <FontAwesomeIcon icon={faWarehouse} className={action.text} />
            변경 요약
          </h3>
...
const MaterialLogPage: React.FC = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const unsubscribe = materialLogService.subscribeRecentLogs(
      (nextLogs) => {
        setLogs(nextLogs);
        setLoading(false);
      },
  ...
  const loadOnce = async () => {
    setLoading(true);
    try {
      const nextLogs = await materialLogService.getRecentLogs(500);
      setLogs(nextLogs);
      setError(null);
    } catch (err) {
    ...
    <div className="mx-auto max-w-[1800px] p-6">
      <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <h1 className="mt-3 text-3xl font-black text-slate-900">자재관리 변경 로그</h1>
          <p className="mt-2 text-sm text-slate-500">
            자재 마스터, 입고, 출고가 언제 누가 어떻게 저장·수정·삭제됐는지 변경 전후값까지 추적합니다.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-5">
        <StatCard label="전체 로그" value={stats.total.toLocaleString('ko-KR')} icon={faWarehouse} tone="bg-slate-100 text-slate-700" />
      ...
      <div className="mb-6 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-3 xl:grid-cols-[1.2fr_0.75fr_0.75fr_0.75fr_0.75fr_0.75fr]">
          ...
          >
            ...
            <option value="material">자재 마스터</option>
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-white text-xs font-black uppercase tracking-wide text-slate-400">
                <tr>
                  ...
                  <th className="px-4 py-3">자재/거래</th>
                  ...
                  return (
                    <React.Fragment key={log.id}>
                      ...
                      >
                        ...
                        <td className="px-4 py-4">
                          <div className="font-bold text-slate-900">{log.materialName}</div>
                          <div className="mt-1 flex flex-wrap gap-2 text-xs text-slate-500">
                            <span>{log.category || '-'}</span>
                            <span>{log.spec || '-'}</span>
                            {log.siteName && <span>{log.siteName}</span>}
                            {log.transactionDate && <span>{log.transactionDate}</span>}
                          </div>
                        </td>
                        <td className="px-4 py-4">
