...
it('shows the scope and explains the problem before opening its source', () => {
    ...
    expect(screen.getByRole('link', { name: '숙소 원장 대조 열기' })).toHaveAttribute('href', '/support/accommodation?view=ledger');
    expect(screen.getByRole('link', { name: '차량 원장 대조 열기' })).toHaveAttribute('href', '/support/vehicles?view=ledger');
