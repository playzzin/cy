...
const TaxInvoicePage: React.FC = () => {
    ...
    const [invoicer, setInvoicer] = useState({
        corpNum: COMPANY_INFO.corpNum,
        corpName: COMPANY_INFO.corpName,
        ceoName: COMPANY_INFO.ceoName,
        addr: COMPANY_INFO.addr,
        bizType: COMPANY_INFO.bizType,
        bizClass: COMPANY_INFO.bizClass,
    ...
    // Firestore 데이터 로드 (초기화)
    useEffect(() => {
        const loadData = async () => {
            try {
                // 1. 자사 정보 로드 (Firestore -> Backend -> Config 순)
                ...
                if (savedInfo) {
                    ...
                    // 현재 정책: 저장된 정보가 있으면 그것을 우선하되, 사업자번호가 비어있다면 백엔드 값 사용
                    setInvoicer(prev => ({
                        ...prev,
                        ...savedInfo,
    ...
    const handleSubmit = async () => {
        ...
        const invoicerCorpNum = normalizeDigits(invoicer.corpNum);
        const invoiceeCorpNum = normalizeDigits(invoicee.corpNum);

        // 유효성 검사
        if (!invoicerCorpNum || !invoiceeCorpNum) {
            setResult({ success: false, message: '사업자번호를 입력해주세요.' });
            return;
        }

        if (invoicerCorpNum.length !== 10 || invoiceeCorpNum.length !== 10) {
            setResult({ success: false, message: '사업자번호는 숫자 10자리여야 합니다.' });
            return;
        }

        if (!invoicer.corpName || !invoicer.ceoName) {
            setResult({ success: false, message: '공급자 상호/대표자명을 입력해주세요.' });
            return;
        }

        if (!invoicee.corpName || !invoicee.ceoName) {
            setResult({ success: false, message: '공급받는자 상호/대표자명을 입력해주세요.' });
            return;
        }
        ...
        try {
            const writeDateYyyyMmDd = formatDateForTaxInvoice(new Date(writeDate));
            ...
            const requestItems = items
                ...
                .map((item, idx) => ({
                    ...item,
                    serialNum: idx + 1,
                    purchaseDT: item.purchaseDT || writeDateYyyyMmDd,
                }));
            ...
            const request: TaxInvoiceRequest = {
                ...
                invoiceeHP: invoicee.hp ? normalizeDigits(invoicee.hp) : undefined,

                writeDate: writeDateYyyyMmDd,
                supplyCostTotal,
                taxTotal,
                totalAmount,
            ...
            if (response.success) {
                // 공급자 정보 저장 (다음 번 사용을 위해)
                try {
                    await companyService.saveMyCompanyInfo(invoicer);
                } catch (e) {
                    console.error('공급자 정보 저장 실패:', e);
                }

                // 미수금 관리: 세금계산서 거래원장 조회 페이지에서 '미수금 등록' 기능을 통해 수동으로 등록합니다.
                            ...
                            <div className="bg-white rounded-xl shadow-sm border p-6">
                                ...
                                <div>
                                    ...
                                    <input
                                        type="text"
                                        value={invoicee.hp}
                                        onChange={(e) => setInvoicee({ ...invoicee, hp: e.target.value })}
