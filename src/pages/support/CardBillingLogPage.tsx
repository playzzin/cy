...
import { cardBillingLogService } from '../../services/cardBillingLogService';
import type {
...
type ActionFilter = 'all' | CardBillingLogAction;

const ACTION_META: Record<CardBillingLogAction, { label: string; icon: any; badge: string }> = {
  created: {
    label: '저장',
    icon: faPlus,
...
const buildSearchText = (log: CardBillingLog): string => [
  log.billingId,
  log.yearMonth,
  log.cardLabel,
  log.teamName,
...
const FieldChangeList: React.FC<{ changes: CardBillingFieldChange[] }> = ({ changes }) => {
  if (changes.length === 0) {
...
const LineItemChangeTable: React.FC<{
  title: string;
  rows: CardBillingLineItemChange[];
  emptyText: string;
  tone: string;
...
const DetailPanel: React.FC<{ log: CardBillingLog; onClose: () => void }> = ({ log, onClose }) => {
    ...
    <aside className="rounded-xl border border-slate-200 bg-white shadow-sm">
      <div className="flex items-start justify-between gap-4 border-b border-slate-200 bg-slate-50 px-5 py-4">
        <div>
          ...
          <h2 className="mt-3 text-xl font-black text-slate-900">{log.cardLabel} 카드 청구 상세</h2>
...
const CardBillingLogPage: React.FC = () => {
  const [logs, setLogs] = useState<CardBillingLog[]>([]);
  ...
  useEffect(() => {
    setLoading(true);
    const unsubscribe = cardBillingLogService.subscribeRecentLogs(
      (nextLogs) => {
        setLogs(nextLogs);
        setLoading(false);
      },
  ...
  const loadOnce = async () => {
    setLoading(true);
    try {
      const nextLogs = await cardBillingLogService.getRecentLogs(500);
      setLogs(nextLogs);
      setError(null);
    } catch (err) {
      ...
      <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-black text-slate-500 shadow-sm">
            ...
            Card Billing Audit Trail
          </div>
          <h1 className="mt-3 text-3xl font-black text-slate-900">카드 청구 변경 로그</h1>
          <p className="mt-2 text-sm text-slate-500">
            언제, 누가, 어떤 카드 청구서를 저장·수정·확정·취소했는지 변경 전후 값까지 추적합니다.
          </p>
        </div>
        <button
          type="button"
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-white text-xs font-black uppercase tracking-wide text-slate-400">
                <tr>
                  ...
                  <th className="px-4 py-3">카드 청구서</th>
                  ...
                  return (
                    <React.Fragment key={log.id}>
                      ...
                      >
                        ...
                        <td className="px-4 py-4">
                          <div className="font-bold text-slate-900">{log.cardLabel || '-'}</div>
                          <div className="mt-1 flex flex-wrap gap-2 text-xs text-slate-500">
                            <span><FontAwesomeIcon icon={faCreditCard} /> {log.yearMonth || '-'}</span>
...
export default CardBillingLogPage;
