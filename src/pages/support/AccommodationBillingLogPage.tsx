...
import { accommodationBillingLogService } from '../../services/accommodationBillingLogService';
import type {
...
type ActionFilter = 'all' | AccommodationBillingLogAction;

const ACTION_META: Record<AccommodationBillingLogAction, { label: string; icon: any; badge: string }> = {
  created: {
    label: '저장',
    icon: faPlus,
...
const buildSearchText = (log: AccommodationBillingLog): string => [
  log.billingId,
  log.yearMonth,
  log.teamName,
...
const FieldChangeList: React.FC<{ changes: AccommodationBillingFieldChange[] }> = ({ changes }) => {
  if (changes.length === 0) {
...
const LineItemChangeTable: React.FC<{
  title: string;
  rows: AccommodationBillingLineItemChange[];
  emptyText: string;
  tone: string;
...
const DetailPanel: React.FC<{ log: AccommodationBillingLog; onClose: () => void }> = ({ log, onClose }) => {
    ...
    <aside className="rounded-xl border border-slate-200 bg-white shadow-sm">
      <div className="flex items-start justify-between gap-4 border-b border-slate-200 bg-slate-50 px-5 py-4">
        <div>
          ...
          <h2 className="mt-3 text-xl font-black text-slate-900">{log.issuedToWorkerName || log.teamName || '청구대상'} 숙소 청구 상세</h2>
          <p className="mt-1 text-sm text-slate-500">{log.yearMonth} · {log.teamName || '팀 미지정'}</p>
        </div>
        <button
          type="button"
          onClick={onClose}
...
const AccommodationBillingLogPage: React.FC = () => {
  const [logs, setLogs] = useState<AccommodationBillingLog[]>([]);
  ...
  useEffect(() => {
    setLoading(true);
    const unsubscribe = accommodationBillingLogService.subscribeRecentLogs(
      (nextLogs) => {
        setLogs(nextLogs);
        setLoading(false);
      },
  ...
  const loadOnce = async () => {
    setLoading(true);
    try {
      const nextLogs = await accommodationBillingLogService.getRecentLogs(500);
      setLogs(nextLogs);
      setError(null);
    } catch (err) {
      ...
      <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-black text-slate-500 shadow-sm">
            ...
            Accommodation Billing Audit Trail
          </div>
          <h1 className="mt-3 text-3xl font-black text-slate-900">숙소 청구 변경 로그</h1>
          <p className="mt-2 text-sm text-slate-500">
            언제, 누가, 어떤 숙소 청구서를 저장·수정·확정·취소했는지 변경 전후 값까지 추적합니다.
          </p>
        </div>
        <button
          type="button"
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-white text-xs font-black uppercase tracking-wide text-slate-400">
                <tr>
                  ...
                  <th className="px-4 py-3">숙소 청구서</th>
                  ...
                  return (
                    <React.Fragment key={log.id}>
                      ...
                      >
                        ...
                        <td className="px-4 py-4">
                          <div className="font-bold text-slate-900">{log.issuedToWorkerName || log.teamName || '-'}</div>
                          <div className="mt-1 flex flex-wrap gap-2 text-xs text-slate-500">
                            <span><FontAwesomeIcon icon={faBuilding} /> {log.yearMonth || '-'}</span>
                            <span><FontAwesomeIcon icon={faUsers} /> {log.teamName || '팀 미지정'}</span>
                            {log.status && <span>{STATUS_LABELS[log.status] || log.status}</span>}
                            <span>{log.totalAmount.toLocaleString('ko-KR')}원</span>
                          </div>
                        </td>
                        <td className="px-4 py-4">
...
export default AccommodationBillingLogPage;
