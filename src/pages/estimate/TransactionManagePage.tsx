        ...
        <div className="flex flex-col h-screen overflow-hidden bg-slate-50 font-sans text-slate-900">
            ...
            <header className="flex flex-none items-center justify-between border-b bg-white px-6 py-4 shadow-sm z-30 print:hidden">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-[16px] font-black tracking-tight text-slate-900 leading-tight">거래명세표 관리</h1>
                        <p className="text-[10px] font-bold text-slate-400 tracking-wider uppercase">Transaction Management</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                        <div className="flex overflow-hidden rounded-xl border-2 border-slate-200 bg-white">
                            <button onClick={() => setTransactionMode('standard')} className={`px-3 py-2.5 text-[12px] font-black transition-all ${!isRentalTransaction ? 'bg-slate-900 text-white' : 'text-slate-500 hover:bg-slate-50'}`}>
                                일반
                            </button>
                            <button onClick={() => setTransactionMode('rental')} className={`px-3 py-2.5 text-[12px] font-black transition-all ${isRentalTransaction ? 'bg-amber-600 text-white' : 'text-slate-500 hover:bg-slate-50'}`}>
                                임대
                            </button>
                        </div>
                        <button onClick={() => resetPage('standard')} className="inline-flex items-center gap-2 rounded-xl border-2 border-slate-200 bg-white px-4 py-2.5 text-[12px] font-black text-slate-600 hover:border-teal-500 hover:text-teal-600 transition-all hover:shadow-md">
                            <FontAwesomeIcon icon={faPlus} /> 새 명세표
                        </button>
                        <button onClick={() => resetPage('rental')} className="inline-flex items-center gap-2 rounded-xl border-2 border-amber-200 bg-amber-50 px-4 py-2.5 text-[12px] font-black text-amber-700 hover:bg-amber-600 hover:text-white transition-all hover:shadow-md">
                        ...
                        <button onClick={saveTransaction} disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-slate-900 text-white px-6 py-2.5 text-[12px] font-black hover:bg-teal-600 disabled:opacity-50 shadow-xl transition-all active:scale-95">
            ...
            <div className="flex flex-1 overflow-hidden">
                    ...
                    <div className="p-5 border-b bg-slate-50/50 space-y-4">
                        <div className="relative group">
                            ...
                            <input 
                                ...
                                className="w-full pl-10 pr-4 py-3 text-[13px] bg-white border border-slate-200 rounded-2xl focus:border-teal-500 focus:ring-4 focus:ring-teal-500/5 transition-all font-bold placeholder:text-slate-300 shadow-sm" 
                            />
                        </div>
                            ...
                            return (
                                <div key={e.id} onClick={() => setDraft({ ...getEmptyDraft('transaction'), ...e, items: e.items || [] })} className={`group relative p-4 rounded-2xl border-2 transition-all cursor-pointer hover:shadow-lg active:scale-[0.98] ${draft.id === e.id ? 'bg-teal-50 border-teal-500 shadow-teal-100/50' : 'bg-white border-slate-50 hover:border-slate-200'}`}>
                                    <div className="flex items-start justify-between mb-2">
                                        <h3 className={`text-[14px] font-black leading-tight flex-1 pr-6 ${draft.id === e.id ? 'text-teal-900' : 'text-slate-800'}`}>{e.projectName || e.title || '제목 없음'}</h3>
                                    ...
                                    <div className="space-y-1.5">
                                        <div className="flex items-center gap-2">
                                            <span className={`rounded-lg px-2 py-0.5 text-[10px] font-black ${e.estimateMode === 'rental' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-500'}`}>
                                    ...
                                    <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between">
                                        <span className="text-[13px] font-black text-teal-600">{formatCurrency(e.total)} <span className="text-[10px] font-bold opacity-70">원</span></span>
                ...
                <main className="flex-1 overflow-auto bg-slate-50/50 flex flex-col min-w-0 print:block print:overflow-visible print:bg-white print:p-0 print:m-0 no-scrollbar">
                    <div className="flex-1 flex flex-col items-center p-10 print:p-0">
                        {isRentalTransaction && (
                            <section className="mb-6 w-full max-w-7xl rounded-2xl border border-amber-200 bg-white p-5 shadow-sm print:hidden">
                                <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
                                    <div className="min-w-0">
                                        <div className="flex items-center gap-2">
                                            <h2 className="text-[15px] font-black text-slate-900">임대 금액 자동 생성</h2>
                                    ...
                                    <div className="grid w-full grid-cols-2 gap-3 md:grid-cols-6 xl:w-auto xl:min-w-[980px]">
                                        <label className="col-span-2 md:col-span-1">
                                            <span className="mb-1 block text-[11px] font-black text-slate-500">임대 품목</span>
                                            <select
                                                ...
                                                className="h-10 w-full rounded-lg border border-slate-200 px-3 text-sm font-bold outline-none focus:border-amber-500"
                                            >
                                        ...
                                        <label className="col-span-2 md:col-span-1">
                                            ...
                                            <input
                                                ...
                                                inputMode="numeric"
                                                className="h-10 w-full rounded-lg border border-slate-200 px-3 text-right text-sm font-black outline-none focus:border-amber-500 focus:ring-4 focus:ring-amber-100"
                                                placeholder="0"
                                            />
                                        </label>
                                        <label>
                                        ...
                                        <label>
                                            ...
                                            <input
                                                ...
                                                className="h-10 w-full rounded-lg border border-slate-200 px-3 text-right text-sm font-bold outline-none focus:border-amber-500"
                                            />
                                        </label>
                                        <label>
                                    ...
                                    <div className="mt-5 border-t border-slate-100 pt-4">
                                        <div className="mb-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                                            <input
                                                ...
                                                className="h-10 w-full rounded-lg border border-slate-200 px-3 text-sm font-bold outline-none focus:border-amber-500 md:max-w-sm"
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                <button
                                                    ...
                                                    className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-[12px] font-black text-amber-700 hover:bg-amber-100"
                                                >
                                                    단가 초기화
                                                </button>
                                            </div>
                        ...
                        <div className="mx-auto w-full max-w-7xl bg-white p-12 shadow-2xl shadow-slate-200/50 border border-slate-200 rounded-[2rem] print:border-none print:shadow-none print:p-0 print:max-w-none print:rounded-none">
                            ...
                            <div className="mt-6 p-6 border-2 border-dashed border-slate-200 rounded-2xl text-center font-black text-slate-400 text-[16px] tracking-widest uppercase">위 금액을 정히 영수(청구)함</div>
                        </div>
                    </div>
                </main>
            </div>
