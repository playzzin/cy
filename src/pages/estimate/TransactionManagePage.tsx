...
const TransactionManagePage: React.FC = () => {
    ...
    const handleExcelDownload = async () => {
        try {
            await downloadEstimateExcel(draft, itemsWithCalc, subtotal, tax, total, 'transaction');
            Swal.fire({
                icon: 'success',
                title: '엑셀 변환 완료',
    ...
    const saveTransaction = async () => {
        ...
        try {
            setSaving(true);
            const payload = { ...draft, documentType: 'transaction' as DocumentType, items: itemsWithCalc, subtotal, total };
            if (draft.id) {
            ...
            <header className="flex flex-none items-center justify-between border-b bg-white px-6 py-4 shadow-sm z-30 print:hidden">
                ...
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                        <button onClick={resetPage} className="inline-flex items-center gap-2 rounded-xl border-2 border-slate-200 bg-white px-4 py-2.5 text-[12px] font-black text-slate-600 hover:border-teal-500 hover:text-teal-600 transition-all hover:shadow-md">
