...
export default function WorkbookPreview({ source, target, outputs = [] }: { source?: WorkbookFile; target?: WorkbookFile; outputs?: ConversionOutput[] }) {
  ...
  const [fileIndex, setFileIndex] = useState(0), [sheetIndex, setSheetIndex] = useState(0), [zoom, setZoom] = useState(85);
  ...
  const content = (expanded: boolean) => <>
    <div className="wp-toolbar"><div className="wp-tabs" role="tablist" aria-label="미리보기 종류">{([['source','원본'],['target','받을 양식'],['result','완성 결과']] as [Stage,string][]).map(([value,label]) => <button key={value} role="tab" aria-selected={stage === value} disabled={value === 'source' ? !source : value === 'target' ? !target : !outputs.length} onClick={() => { setStage(value); setFileIndex(0); setSheetIndex(0); }}>{label}</button>)}</div><div className="wp-controls">{files.length > 1 && <select aria-label="미리보기 문서 선택" value={fileIndex} onChange={e => {setFileIndex(Number(e.target.value));setSheetIndex(0);}}>{files.map((f,i) => <option key={f.name} value={i}>{i+1}. {f.name}</option>)}</select>}{!!file && file.sheets.length > 1 && <select aria-label="미리보기 시트 선택" value={sheetIndex} onChange={e => setSheetIndex(Number(e.target.value))}>{file.sheets.map((s,i) => <option key={s.name} value={i}>{s.name}</option>)}</select>}<select aria-label="미리보기 배율" value={zoom} onChange={e => setZoom(Number(e.target.value))}>{[60,85,100,125,150].map(n => <option key={n} value={n}>{n}%</option>)}</select>{!expanded && <button onClick={() => setExpanded(true)}><Maximize2 size={14}/>크게 보기</button>}</div></div>
    <div className="wp-file-caption">{file?.name} · {sheet?.name}</div>
    <div className={`wp-paper-scroll ${expanded ? 'expanded' : ''}`}><div style={{zoom:zoom/100} as React.CSSProperties}>{sheet && <DocumentSheet key={`${stage}-${file?.name}-${sheet.name}`} sheet={sheet}/>}</div></div>
...
export function DocumentSheet({ sheet }: { sheet: SheetInfo }) {
  ...
  const columns = Array.from({length: Math.min(100,sheet.columnCount)},(_,i) => i+1).filter(c => !sheet.hiddenColumns.includes(c));
