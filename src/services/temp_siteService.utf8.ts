...
export interface Site {
    ...
    companyId?: string; // ID of the constructor company (?쒓났?? - Main company
    companyName?: string; // Name of the constructor company
    clientCompanyId?: string; // ID of the client company (諛쒖＜?? - New
    clientCompanyName?: string; // Name of the client company (諛쒖＜?? - New
    ...
    partnerId?: string; // ?묐젰??ID (Partner) - New
    partnerName?: string; // ?묐젰??(Partner) - New
    createdAt?: Timestamp;
    updatedAt?: Timestamp;
    totalManDay?: number; // ?꾩쟻 怨듭닔
...
export const siteService = {
    addSite: async (site: Partial<Site> & Pick<Site, 'name' | 'code' | 'address' | 'status'>): Promise<string> => {
        const docRef = await addDoc(collection(db, COLLECTION_NAME), {
            ...
            companyId: site.companyId || '', // Constructor (?쒓났??
            companyName: site.companyName || '', // Constructor Name
            clientCompanyId: site.clientCompanyId || '', // Client (諛쒖＜??
        ...
        // Sync: Add Site ID to Client Company (諛쒖＜?? if selected
