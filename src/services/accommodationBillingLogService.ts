...
import type {
  AccommodationBillingChangeSet,
  AccommodationBillingFieldChange,
  AccommodationBillingLineItemChange,
  AccommodationBillingLog,
  AccommodationBillingLogAction,
  AccommodationBillingLogActor,
  CreateAccommodationBillingLogInput,
} from '../types/accommodationBillingLog';

const COLLECTION_NAME = 'accommodation_billing_logs';

const ACTION_LABELS: Record<AccommodationBillingLogAction, string> = {
  created: '저장',
  updated: '수정',
  deleted: '삭제',
};
...
const BILLING_FIELD_LABELS: Record<string, string> = {
  ...
  status: '상태',
  memo: '메모',
  postedAdvancePaymentId: '대납출력부 연동 ID',
  confirmedAt: '확정일시',
};
...
const LINE_ITEM_FIELD_LABELS: Record<string, string> = {
  ...
  targetField: '청구 필드',
  sourceType: '출처',
  sourceAccommodationId: '숙소 ID',
  sourceUtilityRecordId: '공과금 행 ID',
};
...
const resolveActor = (): AccommodationBillingLogActor => {
...
const getTotalAmount = (billing?: Partial<AccommodationBillingDocument> | null): number =>
  (billing?.lineItems || []).reduce((sum, item) => sum + (Number.isFinite(item.amount) ? item.amount : 0), 0);

const snapshotLineItem = (item: AccommodationBillingLineItem): AccommodationBillingLineItem => stripUndefined({
  id: item.id,
  label: item.label,
...
const snapshotBilling = (billing?: Partial<AccommodationBillingDocument> | null): Partial<AccommodationBillingDocument> | null => {
  ...
  return stripUndefined({
    ...
    postedAdvancePaymentId: billing.postedAdvancePaymentId,
  }) as Partial<AccommodationBillingDocument>;
};

const getBillingId = (billing?: Partial<AccommodationBillingDocument> | null): string =>
  asText(billing?.id, '');

const resolveAnchor = (
  before: Partial<AccommodationBillingDocument> | null,
  after: Partial<AccommodationBillingDocument> | null
): Partial<AccommodationBillingDocument> => after || before || {};

const buildFieldChanges = (
  before: Partial<AccommodationBillingDocument> | null,
  after: Partial<AccommodationBillingDocument> | null
): AccommodationBillingFieldChange[] =>
  BILLING_COMPARE_FIELDS.reduce<AccommodationBillingFieldChange[]>((changes, field) => {
...
const getLineItemKey = (item: Partial<AccommodationBillingLineItem>): string =>
  asText(item.id || `${item.label}-${item.targetField}-${item.sourceAccommodationId}-${item.sourceUtilityRecordId}`, '');

const buildLineItemChange = (
  item: Partial<AccommodationBillingLineItem>,
  before?: Partial<AccommodationBillingLineItem>,
  after?: Partial<AccommodationBillingLineItem>
): AccommodationBillingLineItemChange => ({
...
const buildLineItemChanges = (
  beforeItems: AccommodationBillingLineItem[] = [],
  afterItems: AccommodationBillingLineItem[] = []
): AccommodationBillingChangeSet['lineItemChanges'] => {
  ...
  const added: AccommodationBillingLineItemChange[] = [];
  const removed: AccommodationBillingLineItemChange[] = [];
  const updated: AccommodationBillingLineItemChange[] = [];
  ...
  afterByKey.forEach((after, key) => {
    ...
    const changes = LINE_ITEM_COMPARE_FIELDS.reduce<AccommodationBillingFieldChange[]>((acc, field) => {
...
const buildSummaryLines = (
  ...
  anchor: Partial<AccommodationBillingDocument>,
  fieldChanges: AccommodationBillingFieldChange[],
  lineItemChanges: AccommodationBillingChangeSet['lineItemChanges']
): string[] => {
  const lines: string[] = [];
  const targetName = asText(anchor.issuedToWorkerName || anchor.teamName, '청구대상 미지정');
  const totalAmount = formatAmount(getTotalAmount(anchor));

  if (action === 'created') lines.push('숙소 청구서가 생성되었습니다.');
  if (action === 'updated') {
    lines.push(
      fieldChanges.length > 0 || lineItemChanges.added.length > 0 || lineItemChanges.removed.length > 0 || lineItemChanges.updated.length > 0
        ? '숙소 청구서 내용이 수정되었습니다.'
        : '숙소 청구서 저장 값이 갱신되었습니다.'
    );
  }
  if (action === 'deleted') lines.push('숙소 청구서가 취소 또는 삭제되었습니다.');

  lines.push(`청구대상: ${targetName}`);
  lines.push(`팀: ${asText(anchor.teamName, '-')}`);
...
const buildAccommodationBillingChangeSet = (
  action: AccommodationBillingLogAction,
  before: Partial<AccommodationBillingDocument> | null,
  after: Partial<AccommodationBillingDocument> | null
): AccommodationBillingChangeSet => {
...
const normalizeLog = (id: string, data: DocumentData): AccommodationBillingLog => ({
  id,
  action: data.action || 'updated',
  actionLabel: data.actionLabel || ACTION_LABELS[data.action as AccommodationBillingLogAction] || '변경',
  ...
  issuedToType: data.issuedToType ? String(data.issuedToType) as AccommodationBillingDocument['issuedToType'] : undefined,
  ...
  status: data.status ? String(data.status) as AccommodationBillingDocument['status'] : undefined,
  ...
  source: String(data.source || 'accommodationBillingService'),
...
export const accommodationBillingLogService = {
  collectionName: COLLECTION_NAME,
  actionLabels: ACTION_LABELS,
  buildChangeSet: buildAccommodationBillingChangeSet,

  createLog: async (input: CreateAccommodationBillingLogInput): Promise<AccommodationBillingLog> => {
    const before = snapshotBilling(input.before);
    const after = snapshotBilling(input.after);
    const anchor = resolveAnchor(before, after);
    const changeSet = buildAccommodationBillingChangeSet(input.action, before, after);
    const now = Timestamp.now();
    const logRef = doc(collection(db, COLLECTION_NAME));

    const log: AccommodationBillingLog = {
      id: logRef.id,
      action: input.action,
    ...
    try {
      ...
      await systemMessageService.notifyAccommodationBillingLogEvent(log);
    } catch (error) {
      console.warn('[accommodationBillingLogService] accommodation billing log notification failed:', error);
    }
  ...
  subscribeRecentLogs: (
    callback: (logs: AccommodationBillingLog[]) => void,
    limitCount = 300,
    ...
    return onSnapshot(
      ...
      (error) => {
        console.error('[accommodationBillingLogService] subscribe failed:', error);
        onError?.(error);
      }
    );
  },

  getRecentLogs: async (limitCount = 300): Promise<AccommodationBillingLog[]> => {
    const logsQuery = query(collection(db, COLLECTION_NAME), orderBy('createdAt', 'desc'), limit(limitCount));
