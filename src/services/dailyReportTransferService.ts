...
const normalizeWorkerPayload = (value: unknown): DailyReportWorker | null => {
    ...
    return {
        ...
        unitPrice,
        payType: toOptionalText(raw.payType),
        salaryModel: toOptionalText(raw.salaryModel ?? raw.payType),
...
export const dailyReportTransferService = {
    ...
    async importRowsFromExcel(file: File, context: DailyReportExcelImportContext): Promise<DailyReportExcelImportResult> {
        ...
        rows.forEach((row, index) => {
            ...
            group.workers.set(String(worker.id), {
                ...
                payType: worker.payType ?? (salaryModel || undefined),
                salaryModel: salaryModel || worker.salaryModel || worker.payType || undefined,
                siteType: siteType || undefined,
                paymentType: paymentType || undefined,
