...
import type {
    AnalyzeVehicleFineFileInput,
    AnalyzeVehicleFineNoticesInput,
    AnalyzeVehicleFineNoticesResult,
    CommitVehicleFineImportsInput,
    CommitVehicleFineImportsResult,
    VehicleFineAnalysisProgress,
    VehicleFineNoticeAnalysis,
} from '../types/vehicleFineImport';
import { normalizeVehicleFineAnalysis } from '../utils/vehicleFineImportReview';
...
const createOperationId = (): string => {
    ...
    return `vehicle-fine-import:${new Date().toISOString()}:${random}`;
};

export const vehicleFineImportService = {
    validateFiles(files: File[]): void {
        if (files.length === 0) throw new Error('분석할 과태료 고지서를 선택해 주세요.');
    ...
    async analyzeFiles(
        files: File[],
        onProgress?: (progress: VehicleFineAnalysisProgress) => void,
        vehiclePlates: string[] = [],
    ): Promise<VehicleFineNoticeAnalysis[]> {
        this.validateFiles(files);
        aiSettingsService.assertPathEnabled('/support/vehicles', '차량 과태료 AI 분석');

        const callable = httpsCallable<AnalyzeVehicleFineNoticesInput, AnalyzeVehicleFineNoticesResult>(
            functions,
            'analyzeVehicleFineNotices',
            { timeout: ANALYSIS_TIMEOUT_MS },
        );
        const analyses: VehicleFineNoticeAnalysis[] = [];
        const batches = buildBatches(files);
        const plateHints = Array.from(new Set(vehiclePlates
            .map((plate) => String(plate || '').normalize('NFKC').trim())
            .filter(Boolean)))
            .slice(0, 300);
        ...
        const analyzeNextBatch = async (): Promise<void> => {
            ...
            reportProgress();
            const encodedFiles: AnalyzeVehicleFineFileInput[] = await Promise.all(batch.map(async ({ file, fileIndex }) => ({
                fileIndex,
            ...
            try {
                const response = await callable({ files: encodedFiles, vehiclePlates: plateHints });
                analyses.push(...(response.data.notices || []).map(normalizeVehicleFineAnalysis));
            } catch (error) {
                throw new Error(getErrorMessage(error, '과태료 고지서 분석에 실패했습니다.'));
            } finally {
        ...
        return analyses.sort((left, right) => left.fileIndex - right.fileIndex);
    },

    createOperationId,

    async commit(input: CommitVehicleFineImportsInput): Promise<CommitVehicleFineImportsResult> {
        const callable = httpsCallable<CommitVehicleFineImportsInput, CommitVehicleFineImportsResult>(
            functions,
        ...
        try {
            ...
            throw new Error(getErrorMessage(error, '과태료 대장 등록에 실패했습니다.'));
        }
    },
};
