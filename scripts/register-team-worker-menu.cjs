...
const PATH = '/manpower/team-worker-requests';
...
function addChild(parent, text, previousLabel, makeId) {
  ...
  else parent.sub.push({ id: makeId(), text, path: PATH, icon: 'faUserPlus', hide: false });
}
function registerTeamWorkerMenu(input, makeId = () => `menu-${randomUUID()}`) {
  ...
  const expense = find(config[siteKey].menu, '인원관리');
  if (!expense) throw new Error('팀장 인원관리 메뉴를 찾을 수 없습니다.');
  addChild(expense, '신규 작업자 등록', '인원현황', makeId);
  ...
  if (officeApproval) { officeApproval.text = '작업자 등록 승인'; officeApproval.hide = false; }
  else config[officeKey].menu.push({ id: makeId(), text: '작업자 등록 승인', path: PATH, icon: 'faUserPlus', hide: false });
  changedSites.push(officeKey);
  const management = find(config.admin.menu, '운영 관리');
  if (management) { addChild(management, '팀장 작업자 등록 승인', '운영 관리', makeId); changedSites.push('admin'); }
  return { config, changedSites, teamSiteKey: siteKey, officeSiteKey: officeKey };
}
module.exports = { registerTeamWorkerMenu };
if (require.main === module) {
  ...
  (async () => {
    const result = await db.runTransaction(async transaction => {
      ...
      const next = registerTeamWorkerMenu(before);
      if (apply && JSON.stringify(next.config) !== JSON.stringify(before)) {
        const backupDir = path.join(__dirname, '../.codex-team-worker-requests');
      ...
      return { mode: apply ? 'applied' : 'preview', team: next.config[next.teamSiteKey].name, teamWorkerMenu: find(next.config[next.teamSiteKey].menu, '인원관리'), office: next.config[next.officeSiteKey].name, changedSites: next.changedSites };
    });
