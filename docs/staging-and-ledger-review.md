...
확정된 정산은 그대로 읽고 현재 청구와 비교한다. 검사는 어떤 원장·청구·정산도 수정하지 않는다. 사무실 부담은 구분하고, 개인 차량비나 구 방식의 합산 정산처럼 연결 번호가 없는 항목은 추가 확인으로 남긴다. 실제 은행 지급 여부는 검사하지 않는다. 결과는 조회 시점의 자료 기준이며 저장·월 변경·재조회 시 폐기한다.
